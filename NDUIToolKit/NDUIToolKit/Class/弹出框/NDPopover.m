...
@property (nonatomic, assign, readwrite) NDPopoverPosition popoverPosition;
...
@implementation NDPopover {
    BOOL _isiOS7;
}

+ (instancetype)popover {
    return [[NDPopover alloc] init];
}
...
- (instancetype)init {
    ...
    if (self) {
        ...
        self.sideEdge = 10.0;
        self.maskType = NDPopoverMaskTypeBlack;
...
- (void)showAtPoint:(CGPoint)point
     popoverPostion:(NDPopoverPosition)position
    ...
    NSAssert(contentWidth > 0 && contentHeight > 0,
             @"NDPopover contentView bounds.size should not be zero");
    NSAssert(containerWidth > 0 && containerHeight > 0,
             @"NDPopover containerView bounds.size should not be zero");
    NSAssert(containerWidth >= (contentWidth + self.contentInset.left + self.contentInset.right),
             @"NDPopover containerView width %f should be wider than contentViewWidth %f & "
             @"contentInset %@",
    ...
    switch (self.maskType) {
        case NDPopoverMaskTypeBlack:
            maskColor = [UIColor colorWithWhite:0.0 alpha:0.3];
            break;
        case NDPopoverMaskTypeNone: {
...
- (void)showAtView:(UIView *)atView
   ...
   withContentView:(UIView *)contentView
    ...
    NSAssert((upCanContain || downCanContain),
             @"NDPopover no place for the popover show, check atView frame %@ "
    ...
    NDPopoverPosition popoverPosition;
    if (upCanContain) {
        popoverPosition = NDPopoverPositionUp;
        atPoint.y = CGRectGetMinY(atViewFrame) - betweenArrowAndAtView;
    } else {
        popoverPosition = NDPopoverPositionDown;
    ...
    if (upCanContain && downCanContain) {
        ...
        if (position != 0) {
            useUp = position == NDPopoverPositionUp ? YES : NO;
        }
        if (useUp) {
            popoverPosition = NDPopoverPositionUp;
            atPoint.y = CGRectGetMinY(atViewFrame) - betweenArrowAndAtView;
...
- (void)show {
    ...
    CGFloat originY = 0.0;
    if (self.popoverPosition == NDPopoverPositionDown) {

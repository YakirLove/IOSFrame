...
- (id)initWithHeight:(CGFloat)height
{
    ...
    if (self) {
        ...
        UIImage *image = [UIImage imageInUIToolKitProject:@"sr_refresh.tiff"];
        
        _refleshView = [[UIImageView alloc] initWithImage:image];
...
- (void)scrollViewDidScroll
{
    ...
    if (p.y <= - _dragingHeight - _upInset) {
        ...
        if (!_broken) {
            ...
            }else if (self.scrollView.isDragging || isAutomatic==YES) {
                CGPoint ssp = _slime.startPoint;
                _slime.toPoint = CGPointMake(ssp.x, ssp.y + l);
                CGFloat pf = (1.0f-l/_slime.viscous) * (1.0f-kStartTo) + kStartTo;
                _refleshView.layer.transform = CATransform3DMakeScale(pf, pf, 1);
...
- (void)startRefresh {
    isAutomatic = YES;
    [self performSelector:@selector(automaticRefresh) withObject:nil afterDelay:0.0012];
}

-(void)automaticRefresh
{
    if (_scrollView==nil || _scrollView.contentOffset.y<=-87) {
        isAutomatic = NO;
        [self scrollViewDidEndDraging];
        CGPoint p = _scrollView.contentOffset;
        p.y = -32;

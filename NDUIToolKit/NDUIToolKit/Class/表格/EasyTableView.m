...
//	tableView.showsVerticalScrollIndicator	 = NO;
    tableView.scrollIndicatorInsets = UIEdgeInsetsMake(0,0,0,tableView.height - 8);
...
- (void)selectCellAtIndexPath:(NSIndexPath *)indexPath animated:(BOOL)animated {
    ...
    CGFloat height = indexPath.row  *_cellWidthOrHeight;
    if (self.tableView.contentSize.height < height + self.tableView.width) {
        height = self.tableView.contentSize.height - self.tableView.width;
    }
    
    height = height < 0 ? 0 : height;
	CGPoint defaultOffset = CGPointMake(0, height);
